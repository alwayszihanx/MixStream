package io.alwayszihan.mixstream
            ...
            when (call.method) {
                "getAllBuildConfig" -> {
                    ...
                    try {
                        ...
                        map["APPLICATION_ID"] = packageName
                        map["VERSION_CODE"] = pi.versionCode.toString()
                        map["VERSION_NAME"] = pi.versionName ?: ""
                        map["COMPILE_SDK_VERSION"] = android.os.Build.VERSION.SDK_INT.toString()
                        map["MIN_SDK_VERSION"] = ai.minSdkVersion.toString()
                        map["TARGET_SDK_VERSION"] = ai.targetSdkVersion.toString()
                        map["BUILD_TIME"] = ""
                        val sourceFile = java.io.File(ai.sourceDir)
                        map["APP_SIZE"] = sourceFile.length().toString()
                    } catch (e: Exception) {
                ...
                "getAppSize" -> {
                    try {
                        ...
                        result.success(file.length().toString())
                    } catch (e: Exception) {
                        result.success("0")
                    }
                }

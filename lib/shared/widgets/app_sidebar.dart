...
class _AppSidebarState extends ConsumerState<AppSidebar> {
  late final ValueNotifier<double> _mouseX;
  int? _focusedIndex;

  @override
  void initState() {
    super.initState();
    _mouseX = ValueNotifier(double.infinity);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (_focusedIndex != currentFocused) {
        setState(() {
          ...
          if (currentFocused != null && _mouseX.value == double.infinity) {
            ref
  ...
  Widget build(BuildContext context) {
    ...
    // Fixed height container (dock at the bottom of the screen)
    const double dockHeight = 64.0;

    // Static unscaled X coordinates of the item centers for calculating distance
    // Left padding = 16.0, Item base size = 40.0, Gap = 16.0
    ...
    return Material(
      ...
      child: Container(
        height: LayoutConstants.sidebarDockHeight, // 80.0
        alignment: Alignment.center,
        clipBehavior: Clip.none,
        child: ValueListenableBuilder<double>(
          valueListenable: _mouseX,
          builder: (context, mouseXValue, child) {
            ...
            for (int i = 0; i < destinations.length; i++) {
              ...
              if (mouseXValue != double.infinity) {
                distance = mouseXValue - unscaledCenters[i];
            ...
            // 2. Calculate dynamic positioning (lefts) and total dock width to avoid overlaps
            final lefts = List<double>.filled(destinations.length, 0.0);
            lefts[0] = 16.0; // Left padding
            for (int i = 1; i < destinations.length; i++) {
              lefts[i] =
                  lefts[i - 1] +
                  itemSizes[i - 1] +
                  16.0; // Dynamic width + 16.0px gap
            }
            final double dynamicDockWidth =
                lefts[destinations.length - 1] +
            ...
            return MouseRegion(
              onHover: (event) {
                _mouseX.value = event.localPosition.dx;
              ...
              child: AnimatedContainer(
                ...
                height: dockHeight,
                width:
                    dynamicDockWidth, // Dynamic width scales on the spring curve!
                clipBehavior: Clip.none,
                ...
                child: Stack(
                  ...
                  children: List.generate(destinations.length, (i) {
                    ...
                    return AnimatedPositioned(
                      ...
                      curve: const _AceternitySpringCurve(),
                      top:
                          12.0, // Fixed top baseline anchors growth vertically around center
                      left: lefts[i],
                      width: itemSizes[i],
                      height: itemSizes[i],
      ...
      child: MouseRegion(
        ...
        child: SizedBox.expand(
          child: Stack(
            clipBehavior: Clip.none,
            alignment: Alignment
                .bottomCenter, // Centered to the bottom edge of the animated positioned widget
            children: [
              Positioned(
                left: 0,
                right: 0,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(
                      height: 12.0,
                    ), // Constant gap of 12.0px between tooltip and badge!
                    LayoutBuilder(
